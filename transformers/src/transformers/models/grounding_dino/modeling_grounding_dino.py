...
class GroundingDinoDecoderOutput(ModelOutput):
    ...
    # add sampling_locations to decoder output
    sampling_locations: Optional[Tuple[torch.FloatTensor]] = None
...
class GroundingDinoDeformableLayer(nn.Module):
    ...
    def forward(
        ...
        # not storing sampling locations because GroundingDinoDeformableLayer is only used in EncoderLayer
...
class GroundingDinoDecoderLayer(nn.Module):
    def __init__(self, config: GroundingDinoConfig):
        ...
        # cross-attention: visual (extract sampling locations)
    ...
    def forward(
        ...
        hidden_states, cross_attn_weights, sampling_locations = self.encoder_attn(
            hidden_states=hidden_states,
...
class GroundingDinoPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GroundingDinoLearnedPositionEmbedding):
            ...
            # no need to edit this instance because this is only initializing weights
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # layer_outputs: hidden_states, self_attn_weights, text_cross_attn_weights, cross_attn_weights, sampling_locations
            ...
            all_sampling_locations += (layer_outputs[4],)
        ...
        return GroundingDinoDecoderOutput(
            ...
            attentions=all_attns,
            sampling_locations=all_sampling_locations,
        )
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            ...
            tuple_outputs = (
                (decoder_outputs[0], init_reference_points) + decoder_outputs[1:len(decoder_outputs)-1] + encoder_outputs + enc_outputs + (decoder_outputs[-1],)
            )
        ...
        return GroundingDinoModelOutput(
            ...
            sampling_locations=decoder_outputs.sampling_locations,
        )
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            ...
            output = [loss, loss_dict, logits, pred_boxes, *auxiliary_outputs, *outputs[:-1], input_ids, *outputs[-1]]

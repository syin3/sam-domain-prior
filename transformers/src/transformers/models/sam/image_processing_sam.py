    ...
    def generate_crop_boxes(
        ...
        Args:
            ...
            return_tensors (`str`, *optional*, defaults to `pt`):
                If `pt`, returns `torch.Tensor`. If `tf`, returns `tf.Tensor`.
        """
        crop_boxes, normalized_points_per_crop, orig_points_per_crop, cropped_images, input_labels = _generate_crop_boxes(
            image,
            target_size,
            crop_n_layers,
            overlap_ratio,
        ...
        if return_tensors == "pt":
            ...
            normalized_points_per_crop = torch.tensor(normalized_points_per_crop, device=device, dtype=torch.float32 if device.type == "mps" else torch.float64)
            ...
            normalized_points_per_crop = tf.convert_to_tensor(normalized_points_per_crop)
            # cropped_images stays as np
            input_labels = tf.convert_to_tensor(input_labels)
        else:
            raise ValueError("return_tensors must be either 'pt' or 'tf'.")
        return crop_boxes, normalized_points_per_crop, orig_points_per_crop, cropped_images, input_labels
...
def _build_point_grid(n_per_side: int) -> np.ndarray:
    ...
    Not a good place to edit/filter because this is a generic function and no info about the image.
    
    Same as FB's implementation: https://github.com/facebookresearch/segment-anything/blob/dca509fe793f601edb92606367a655c15ac00fdf/segment_anything/utils/amg.py#L179-L186
    But seems a little off, e.g., n_per_side=2 --> [0.25, 0.75] instead of [1/3, 2/3]
    """
...
def _generate_crop_boxes(
    ...
    for i in range(crop_n_layers + 1):
        ...
        points_grid.append(_build_point_grid(n_points)) # 2D grid of points evenly spaced in [0,1]x[0,1]

    crop_boxes, layer_idxs = _generate_per_layer_crops(crop_n_layers, overlap_ratio, original_size)

    cropped_images, normalized_points_grid_per_crop, original_points_grid_per_crop = _generate_crop_images(
        crop_boxes, image, points_grid, layer_idxs, target_size, original_size, input_data_format,
        filter_mask
    )
    crop_boxes = np.array(crop_boxes)
    crop_boxes = crop_boxes.astype(np.float32)
    normalized_points_per_crop = np.array([normalized_points_grid_per_crop])
    normalized_points_per_crop = np.transpose(normalized_points_per_crop, axes=(0, 2, 1, 3))

    input_labels = np.ones_like(normalized_points_per_crop[:, :, :, 0], dtype=np.int64)

    return crop_boxes, normalized_points_per_crop, original_points_grid_per_crop, cropped_images, input_labels
...
def _generate_crop_images(
    ...
    """
    cropped_images = []
    normalized_points_per_crop = []
    original_points_per_crop = []
    for i, crop_box in enumerate(crop_boxes):
        ...
        # ----------------------------------
        # Filter the points if `filter_mask` is given
        # ----------------------------------
        if filter_mask is not None:
            # Overwrite `points` with the filtered subset
            points = _filter_points(points, crop_box, filter_mask)
        
        original_points_per_crop.append(points)

        # Now normalize points for the model’s target size
        normalized_points = _normalize_coordinates(target_size, points, original_size)
        normalized_points_per_crop.append(normalized_points)

    return cropped_images, normalized_points_per_crop, original_points_per_crop


def _filter_points(
    points: np.ndarray,
    crop_box: np.ndarray,
    retain_mask: Optional[np.ndarray],
) -> np.ndarray:
    """
    Keeps only the points that fall inside the 'True' region of `filter_mask`.

    Args:
        points (np.ndarray):
            Nx2 array of [x, y] point coordinates *within the crop* (top-left corner = (0,0)).
        crop_box (np.ndarray or tuple/list):
            The bounding box (left, top, right, bottom) describing where this crop lies in
            the original full image.
        retain_mask (np.ndarray):
            A 2D boolean array with shape (H, W). A value of True means "keep" this point. E.g.,
                (1) uncovered by any mask in 1st pass
                (2) uncertainty too low so no value for sample

    Returns:
        np.ndarray:
            Filtered subset of points that pass the mask check.
    """
    left, top, right, bottom = crop_box  # x1, y1, x2, y2

    # Convert local crop-coords -> full-image coords
    # Round or floor if needed to get integer pixel indices
    x_full = np.round(points[:, 0]).astype(int) + left
    y_full = np.round(points[:, 1]).astype(int) + top

    # First, ensure these are in-bounds for the full mask
    H, W = retain_mask.shape
    in_bounds = (
        (x_full >= 0) & (x_full < W) &
        (y_full >= 0) & (y_full < H)
    )

    # Among the in-bounds points, check whether filter_mask[y, x] is True
    valid = np.zeros(len(points), dtype=bool)
    valid[in_bounds] = retain_mask[y_full[in_bounds], x_full[in_bounds]]

    # Return only the points that are valid
    return points[valid]

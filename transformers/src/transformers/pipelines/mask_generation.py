...
class MaskGenerationPipeline(ChunkPipeline):
    ...
    def _sanitize_parameters(self, **kwargs):
        ...
        if "filter_mask" in kwargs:
            preprocess_kwargs["filter_mask"] = kwargs["filter_mask"]
    ...
    def __call__(self, image, *args, num_workers=None, batch_size=None, **kwargs):
        ...
        # original outputs dict has two keys: masks and scores
        outputs_dict = super().__call__(image, *args, num_workers=num_workers, batch_size=batch_size, **kwargs)
        # also return generated grid points
        return {
            **outputs_dict,
            **{"grid_points": self._stored_points,}
        }
    ...
    def preprocess(
        ...
        crop_boxes, normalized_grid_points, original_grid_points, cropped_images, input_labels = self.image_processor.generate_crop_boxes(
            image, target_size, crops_n_layers, crop_overlap_ratio, points_per_crop, crop_n_points_downscale_factor, 
            device=self.device,
            filter_mask=filter_mask,
        )

        # store generated point grid
        self._stored_points = original_grid_points
        ...
        n_points = normalized_grid_points.shape[1]
        ...
        for i in range(0, n_points, points_per_batch):
            batched_points = normalized_grid_points[:, i : i + points_per_batch, :, :]
            labels = input_labels[:, i : i + points_per_batch]
            
            # account for the last incomplete batch
            # is_last = i == n_points - points_per_batch
            remaining_points = n_points - (i + points_per_batch)
            is_last = remaining_points <= 0
